...
import type { Booking, BookingStatus, CheckInOut, Extension, BookingHistory } from '../../lib/types/booking.types';
...
export function RentalDetailPage() {
  ...
  const loadBookingData = async () => {
    try {
      setIsLoading(true);
      const [bookingData, checkInOutData, extensionData, historyData] = await Promise.all([
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <code className="text-sm bg-gray-100 px-3 py-1 rounded">{booking.id.substring(0, 13)}...</code>
              </div>
            </CardHeader>
            <CardContent>
                ...
                <TabsContent value="details" className="space-y-4 mt-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium text-sm">{booking.userId.substring(0, 13)}...</p>
                    </div>

                    <div>
                      <p className="text-sm text-gray-600">Vehicle ID</p>
                      <p className="font-medium text-sm">{booking.vehicleId.substring(0, 13)}...</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {booking.duration >= 24
                          ? `${Math.floor(booking.duration / 24)} ngày ${booking.duration % 24 > 0 ? `${booking.duration % 24} giờ` : ''}`
                  ...
                  {booking.damageReported && (
                    <>
                      ...
                      <div className="flex items-start gap-3 bg-red-50 p-3 rounded-lg">
                        ...
                        <div>
                          <p className="text-sm text-red-700 font-medium">Đã Báo Cáo Hư Hỏng</p>
                          <p className="text-sm mt-1">Phương tiện có báo cáo hư hỏng</p>
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    checkInOuts.map((checkIn) => (
                      <div key={checkIn.id} className="border rounded-lg p-4">
                        ...
                        <div className="mb-3">
                          ...
                          <p className="text-xs text-gray-500">{checkIn.latitude.toFixed(6)}, {checkIn.longitude.toFixed(6)}</p>
                        </div>
                            ...
                            <div className="grid grid-cols-5 gap-2">
                              {checkIn.images.slice(0, 5).map((img, idx) => (
                                <div key={idx} className="aspect-square bg-gray-100 rounded overflow-hidden">
                                  <img src={img} alt={`Check-in ${idx + 1}`} className="w-full h-full object-cover" />
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {checkIn.damageImages.length > 0 && (
                          <div className="mb-3">
                            <p className="text-sm text-red-600 font-medium mb-2">Hình Ảnh Hư Hỏng ({checkIn.damageImages.length})</p>
                            <div className="grid grid-cols-5 gap-2">
                              {checkIn.damageImages.slice(0, 5).map((img, idx) => (
                                <div key={idx} className="aspect-square bg-red-50 rounded overflow-hidden border border-red-200">
                        ...
                        {!checkIn.verified && (
                          ...
                          >
                            ...
                            Xác Nhận {checkIn.type === 'CHECK_IN' ? 'Nhận Xe' : 'Trả Xe'}
                          </Button>
                        )}
                      </div>
                    ))
                    ...
                    extensions.map((ext) => (
                      <div key={ext.id} className="border rounded-lg p-4">
                        ...
                        <div className="text-sm text-gray-600 mb-3">
                          ...
                          {ext.approvedAt && <p>Duyệt lúc: {formatDateTime(ext.approvedAt)}</p>}
                        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Phí thuê ({formatCurrency(booking.vehicleFeeHour)}/h)</span>
                <span className="font-medium">{formatCurrency(booking.rentalFee)}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{formatCurrency(booking.collateral)}</span>
              </div>
